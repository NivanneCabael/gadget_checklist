...
function insert_new_user()
{
    ...
    $fetched_employee_birthdate		    = mysqli_real_escape_string($db,$_POST['employee_birthdate']);

    $time_input = strtotime($fetched_employee_birthdate);  
    $formatted_date = date('M/d/Y',$time_input);  
    ...
    if(mysqli_num_rows($disabled_employee_result) == 1){
        ...
        if(!$data){
            if($fetched_table_name == 'employee'){
                $insert_new_user = "INSERT INTO employee(employee_id,employee_name,company_id,department_id,position,profile_pic_url,employee_status,birthday,created_at)
                                    VALUES('$uppercased_employee_id','$fetched_employee_name',$fetched_employee_company,$fetched_employee_department,
                                    '$fetched_employee_position','$url',1,'$formatted_date',NOW())";    
    ...
    if(mysqli_num_rows($disabled_employee_result) == 1){
        ...
        if(!$data){
            if($fetched_table_name == 'employee_no_picutre'){
                $insert_new_user = "INSERT INTO employee(employee_id,employee_name,company_id,department_id,position,employee_status,birthday,created_at)
                                    VALUES('$uppercased_employee_id','$fetched_employee_name',$fetched_employee_company,$fetched_employee_department,
                                    '$fetched_employee_position',1,'$formatted_date',NOW())"; 
                $result_insert = mysqli_query($db,$insert_new_user);   
                echo 'Inserted Successfully';                   
            }
            else {
...
function fetch_row_data()
{
    ...
    while ($row = mysqli_fetch_assoc($fetched_user_result)) 
	{
        ...
        if($birthday != NULL){
            $formatted_date = str_replace('/', '-', $birthday);
            $time_input = strtotime($formatted_date);
            $formatted_date = date('Y-m-d',$time_input);  
        }
        else {
            $formatted_date = NULL;
        }
        ...
        // $user_data[6] = date('M/d/Y',$user_data[6]);  

        $user_data[0] = $user_id;
        $user_data[1] = $employee_id;
        $user_data[2] = $employee_name;
        $user_data[3] = $company_id;
        $user_data[4] = $department_id;
        $user_data[5] = $position;
        $user_data[6] = $formatted_date;

        // $user_data[6] = strtotime($user_data[6]);  
        // $user_data[6] = date('M/d/Y',$user_data[6]);  
